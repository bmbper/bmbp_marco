...
use bmbp_marco_util::{build_struct_field_token, build_struct_option_field_token, build_struct_option_props_method_token, build_struct_props_method_token, build_struct_token, build_tree_field, build_tree_field_name, merge_struct_fields, parse_struct_fields};
use crate::meta::RdbcTableTreeMeta;
use crate::util::{build_impl_orm_row_token, build_struct_table_token, build_table_name};


pub(crate) fn marco_table_tree_bean(meta_token: TokenStream, struct_token: TokenStream) -> TokenStream {
...
pub(crate) fn marco_table_tree_bean_option(meta_token: TokenStream, struct_token: TokenStream) -> TokenStream {
    let rdbc_meta = parse_macro_input!(meta_token as RdbcTableTreeMeta);
    let temp_struct_token = struct_token.clone();
    let struct_input = parse_macro_input!(temp_struct_token as syn::DeriveInput);
    let struct_ident = &struct_input.ident;
